...
import { TableContainer, Table, TableHead, TableRow, TableBody, TableCell as UiTableCell, MenuItem as UiMenuItem, Typography, Menu, IconButton, ListItemIcon, Box, Stack, Grid, Alert, TablePagination, TextField, InputAdornment, ToggleButtonGroup, ToggleButton as UiToggleButton, TableSortLabel, Dialog, DialogTitle, DialogContent, DialogActions, Button, Paper, CircularProgress, Autocomplete, RadioGroup, FormControlLabel, Radio, Chip, DialogContentText, Slide, AppBar, Toolbar } from 'src/shared/components/compat';
...
import StoreInvoiceItemsTable from 'src/features/warehouse/pages/list-store-invoice/StoreInvoiceItemsTable';
import DeleteStoreInvoiceModal from 'src/features/warehouse/pages/list-store-invoice/DeleteStoreInvoice';
...
import ListDrivers from 'src/features/drivers/pages/ListDrivers';
import { TransitionProps } from 'src/shared/components/compat';
                                                ...
                                                <Menu id="basic-menu" anchorEl={anchorEl} open={Boolean(anchorEl) && selectedInvoiceForMenu?.id === row.id} onClose={handleCloseMenu} MenuListProps={{ 'aria-labelledby': `basic-button-${row.id}` }}>
                                                    ...
                                                    {hasDeletePermission && selectedInvoiceForMenu?.status === 0 && (
                                                        <CustomTooltip placement="left" title={isTooltipGloballyEnabled ? 'Bu faturayı silin' : ''}>
                                                            <UiMenuItem onClick={() => handleClickOpenDeleteModal(row.id, row.invoiceNo || '-')}>
