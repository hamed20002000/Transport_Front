...
import { FormControl, InputLabel, Select, MenuItem as UiMenuItem, TextField, Button, Stack, Box, Typography, InputAdornment, IconButton, Dialog, DialogActions, DialogContent, DialogContentText, DialogTitle, ListSubheader, Grid } from 'src/shared/components/compat';
                        ...
                        <FormControl fullWidth error={itemError}>
                            ...
                            >
                                {loadingAvailableItems ? (
                                    <UiMenuItem disabled>Yükleniyor...</UiMenuItem>
                                ) : (
                                    selectableItems.length > 0 ? (
                                        selectableItems.map((item) => (
                                            <UiMenuItem key={item.id} value={item.id}>
                                                {item.name}
                                            </UiMenuItem>
                                        ))
                                    ) : (
                                        <UiMenuItem disabled>Hiç öğe bulunamadı.</UiMenuItem>
                                    )
                                )}
                                {itemToEdit && (
                                    <UiMenuItem key={itemToEdit.id} value={itemToEdit.id} disabled>
