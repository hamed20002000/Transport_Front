...
import { Typography, Box, Stack, Grid, Button, Alert, CircularProgress, Paper, Chip, IconButton, TableContainer, Table, TableHead, TableRow, TableBody, Menu, ListItemIcon, TablePagination, TableCell as UiTableCell, MenuItem as UiMenuItem, Dialog, DialogTitle, DialogActions, DialogContent, DialogContentText, Divider, TextField, InputAdornment, ToggleButtonGroup, FormControl, ToggleButton as UiToggleButton, TableSortLabel, Tab, Autocomplete } from 'src/shared/components/compat';
import { TabContext, TabList, TabPanel } from 'src/shared/components/compat';
import { keyframes, styled } from 'src/shared/components/compat';
...
import DeleteRequest from 'src/features/operations/pages/list-request/DeleteRequest';
import DeleteWorkhouseRent from 'src/features/operations/pages/list-request/DeleteWorkhouseRent';
import { useAuth } from "src/core/auth/AuthContext";

import jsPDF from 'jspdf';
import autoTable from 'jspdf-autotable';
import { NotoSansRegular } from 'src/shared/assets/fonts/NotoSans-Regular';
import { TimesNewRoman } from 'src/shared/assets/fonts/Times';
import { ArialFont } from 'src/shared/assets/fonts/Arial';
import Excel from 'exceljs';
import { saveAs } from 'file-saver';
import { format } from 'date-fns';
import { tr } from 'date-fns/locale';
import { DatePicker } from 'src/shared/components/compat';
...
const StyledToggleButton = styled(UiToggleButton)(({ theme }) => ({
    fontSize: '0.7rem',
    padding: '10px 4px',
    lineHeight: 1.2,
    ...
    '&.Ui-selected[data-value="all"]': { backgroundColor: theme.palette.primary.main, '&:hover': { backgroundColor: theme.palette.primary.dark } },
    '&.Ui-selected[data-value="0"]': { backgroundColor: theme.palette.warning.main, '&:hover': { backgroundColor: theme.palette.warning.dark } },
    '&.Ui-selected[data-value="1"]': { backgroundColor: theme.palette.success.main, '&:hover': { backgroundColor: theme.palette.success.dark } },
    '&.Ui-selected[data-value="2"]': { backgroundColor: theme.palette.error.main, '&:hover': { backgroundColor: theme.palette.error.dark } },
}));
                                                    ...
                                                    <Menu anchorEl={anchorEl} open={openMenu && materialSelectedRowForMenu?.id === row.id} onClose={handleCloseMenu}>
                                                        {hasEditPermission && (
                                                            <CustomTooltip placement="left" title={isTooltipGloballyEnabled ? "Bu kaydı düzenle" : ""}>
                                                                <UiMenuItem onClick={() => handleMaterialEditClick(row)} disabled={row.status !== 0}>
                                                        ...
                                                        {hasDeletePermission && (
                                                            <CustomTooltip placement="left" title={isTooltipGloballyEnabled ? "Bu kaydı sil" : ""}>
                                                                <UiMenuItem onClick={() => handleClickOpenDeleteModal(row)} disabled={row.status !== 0}>
                                        ...
                                        paginatedRentalRequestsList.map((row) => (
                                            <TableRow key={row.id} sx={{ '&:last-child td, &:last-child th': { border: 0 } }}>
                                                ...
                                                <StyledTableCell>
                                                    ...
                                                    >
                                                        {hasEditPermission && (<CustomTooltip placement="left" title={isTooltipGloballyEnabled ? "Bu kaydı düzenle" : ""}><UiMenuItem onClick={() => handleRentalEditClick(row)} disabled={row.status !== 0}><ListItemIcon><IconEdit width={18} /></ListItemIcon> Düzenle</UiMenuItem></CustomTooltip>)}
                                                        {hasDeletePermission && (<CustomTooltip placement="left" title={isTooltipGloballyEnabled ? "Bu kaydı sil" : ""}><UiMenuItem onClick={() => handleClickOpenDeleteModal(row)} disabled={row.status !== 0}><ListItemIcon><IconTrash width={18} /></ListItemIcon> Silmek</UiMenuItem></CustomTooltip>)}
                                                        {hasDownloadPermission && (<CustomTooltip placement="left" title={isTooltipGloballyEnabled ? "Kiralama Raporunu İndir" : ""}><UiMenuItem onClick={() => { setRentalSelectedRowForMenu(row); setOpenDownloadMaterialSingleModal(true); }}><ListItemIcon><IconFileDownload width={18} /></ListItemIcon> Bu satırı indir</UiMenuItem></CustomTooltip>)}
                                                    </Menu>
                                                </StyledTableCell>
                                            </TableRow>
                                        ))
