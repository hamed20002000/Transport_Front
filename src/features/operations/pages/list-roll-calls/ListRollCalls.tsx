...
import { TableContainer, Table, TableHead, TableRow, TableBody, TableCell as UiTableCell, MenuItem as UiMenuItem, Typography, Menu, IconButton, ListItemIcon, Box, Stack, Grid, Button, Alert, TablePagination, TextField, InputAdornment, CircularProgress, Paper, Dialog, DialogTitle, DialogContent, DialogActions, TableSortLabel, RadioGroup, FormControlLabel, Radio } from 'src/shared/components/compat';
import { styled, useTheme } from 'src/shared/components/compat';
import {
...
import { TimePicker, LocalizationProvider, DatePicker } from 'src/shared/components/compat';
import { format } from 'date-fns';
import { tr } from 'date-fns/locale';
import axios from 'axios';
import server from 'src/core/config/endpoints.json';
import { useAuth } from 'src/core/auth/AuthContext';
import { CustomTooltip } from 'src/shared/components/tooltip/TooltipContext';
import DeleteRollCalls from 'src/features/operations/pages/list-roll-calls/DeleteRollCalls';
            ...
            <Paper elevation={3} sx={{ p: 3, mb: 4 }}>
                ...
                <Grid container spacing={2} alignItems="flex-end" mb={3}>
                    <Grid item xs={12} sm={4}>
                        ...
                        >
                            <UiMenuItem value="all">Tüm Çalışanlar</UiMenuItem>
                            <UiMenuItem value="workhouse">Şantiye Çalışanları</UiMenuItem>
                        </TextField>
                    </Grid>
                    ...
                    {dailyFilterType === 'workhouse' && (
                        <Grid item xs={12} sm={4}>
                            ...
                            >
                                {workhouses.map((wh) => (
                                    <UiMenuItem key={wh.id} value={wh.id}>{wh.name}</UiMenuItem>
                                ))}
                            </TextField>
                        </Grid>
                    )}
                                    ...
                                    paginatedRollCalls.map((row) => (
                                        <TableRow key={row.id} hover>
                                            ...
                                            <StyledTableCell>
                                                ...
                                                >
                                                    {hasEditPermission && (
                                                        <UiMenuItem onClick={handleOpenEditModal}><ListItemIcon><IconEdit width={18} /></ListItemIcon>Düzenle</UiMenuItem>
                                                    )}
                                                    {hasDeletePermission && (
                                                        <UiMenuItem onClick={handleClickOpenDeleteModal}><ListItemIcon><IconTrash width={18} /></ListItemIcon>Silmek</UiMenuItem>
                                                    )}
                                                    {hasDownloadPermission && (
                                                        <UiMenuItem onClick={() => handleOpenRowDownloadModal(row)}><ListItemIcon><IconFileDownload width={18} /></ListItemIcon>Bu Satırı İndیر</UiMenuItem>
                                                    )}
                                                </Menu>
                                            </StyledTableCell>
                                        </TableRow>
