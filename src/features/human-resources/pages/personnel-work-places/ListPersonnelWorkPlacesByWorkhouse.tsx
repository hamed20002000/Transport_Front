...
import { TableContainer, Table, TableHead, TableRow, TableBody, TableCell as UiTableCell, MenuItem as UiMenuItem, Typography, Menu, IconButton, ListItemIcon, Box, Stack, Grid, Button, Alert, TablePagination, TextField, InputAdornment, CircularProgress, Paper, OutlinedInput, Checkbox, ListItemText, SelectChangeEvent, Dialog, DialogTitle, DialogContent, DialogActions, ToggleButtonGroup, ToggleButton as UiToggleButton, TableSortLabel, Autocomplete, Chip, DialogContentText, Select } from 'src/shared/components/compat';
...
import DeletePersonnelWorkPlaces from 'src/features/human-resources/pages/personnel-work-places/DeletePersonnelWorkPlaces';
                    ...
                    <Paper elevation={3} sx={{ p: 3, mb: 3 }}>
                        <Grid container spacing={2}>
                            ...
                            <Grid item xs={12}>
                                ...
                                >
                                    <UiToggleButton value="single">Tek Tek Ekleme</UiToggleButton>
                                    <UiToggleButton value="bulk">Toplu Ekleme</UiToggleButton>
                                </ToggleButtonGroup>
                            </Grid>
                                    ...
                                    {assignmentMode === 'single' ? (
                                        ...
                                        >
                                            {personnels.map((p) => {
                                                ...
                                                return (
                                                    <UiMenuItem key={p.id} value={p.id}>
                                                    ...
                                                    <Menu anchorEl={anchorEl} open={openMenu && selectedRowForMenu?.id === row.id} onClose={handleCloseMenu}>
                                                        ...
                                                        {hasEditPermission && (
                                                            <CustomTooltip placement="left" title={isTooltipGloballyEnabled ? "Bu kaydı düzenle" : ""}>
                                                                <UiMenuItem onClick={handleEditClick}><ListItemIcon><IconEdit width={18} /></ListItemIcon>Düzenlemek</UiMenuItem>
                                                            </CustomTooltip>
                                                        )}
                                                        {hasDeletePermission && (
                                                            <CustomTooltip placement="left" title={isTooltipGloballyEnabled ? "Bu kaydı sil" : ""}>
                                                                <UiMenuItem onClick={handleClickOpenDeleteModal}><ListItemIcon><IconTrash width={18} /></ListItemIcon>Silmek</UiMenuItem>
                                                            </CustomTooltip>
                                                        )}
                                                        {hasDownloadPermission && (
                                                            <CustomTooltip placement="left" title={isTooltipGloballyEnabled ? "Kaydı indir" : ""}>
                                                                <UiMenuItem onClick={() => handleOpenRowDownloadModal(row)}><ListItemIcon><IconFileDownload width={18} /></ListItemIcon> Bu satırı indir</UiMenuItem>
                                                            </CustomTooltip>
                                                        )}
                                                    </Menu>
                                                </StyledTableCell>
