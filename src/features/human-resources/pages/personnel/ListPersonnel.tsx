...
import { TableContainer, Table, TableHead, TableRow, TableBody, TablePagination, Typography, Chip, Menu, IconButton, ListItemIcon, Box, Grid, Stack, Button, Alert, TextField, InputAdornment, ToggleButtonGroup, ToggleButton as UiToggleButton, TableCell as UiTableCell, MenuItem as UiMenuItem, TableSortLabel, Dialog, DialogTitle, DialogContent, DialogActions, CircularProgress, Autocomplete, RadioGroup, FormControlLabel, Radio, FormLabel, Divider, Stepper, Step, StepLabel, Backdrop, CardMedia, Paper, DialogContentText } from 'src/shared/components/compat';
import { keyframes, styled } from 'src/shared/components/compat';
import { Bolt as BoltIcon } from 'src/shared/components/compat/icons';
import { DoNotDisturbOnRounded as DoNotDisturbOnRoundedIcon } from 'src/shared/components/compat/icons';
import { DoneRounded as DoneRoundedIcon } from 'src/shared/components/compat/icons';
...
import imagedefault from 'src/shared/assets/images/profile/user-d.svg';

import DeletePersonnel from "src/features/human-resources/pages/personnel/DeletePersonnel";
...
const StyledToggleButton = styled(UiToggleButton)(({ theme }) => ({
    ...
    "&.Ui-selected[data-value='active']": { backgroundColor: theme.palette.success.main, "&:hover": { backgroundColor: theme.palette.success.dark } },
    "&.Ui-selected[data-value='inactive']": { backgroundColor: theme.palette.error.main, "&:hover": { backgroundColor: theme.palette.error.dark } },
    "&:not(.Ui-selected)": { color: theme.palette.text.primary, borderColor: theme.palette.divider, "&:hover": { backgroundColor: theme.palette.action.hover } },
}));
                                    ...
                                    <TableRow key={row.id}
                                        ...
                                        <StyledTableCell>
                                            ...
                                            <Menu id="menu" anchorEl={anchorEl} open={openMenu} onClose={handleCloseMenu} MenuListProps={{ "aria-labelledby": `menu-${selectedRowForMenu?.id}` }}>
                                                ...
                                                <UiMenuItem onClick={() => selectedRowForMenu && handleAnnualLeaveClick(selectedRowForMenu)}>
                                                ...
                                                {hasEditPermission && selectedRowForMenu?.recordStatus === 0 ? (
                                                    <CustomTooltip placement="left" title={isTooltipGloballyEnabled ? "Bu projeyi pasif yap" : ""}>
                                                        <UiMenuItem onClick={() => sendStatusUpdate(row.id, 1)}>
                                                    ...
                                                    <CustomTooltip placement="left" title={isTooltipGloballyEnabled ? "Bu projeyi aktif yap" : ""}>
                                                        <UiMenuItem onClick={() => sendStatusUpdate(row.id, 0)}>
                                                ...
                                                {selectedRowForMenu?.attachments && selectedRowForMenu.attachments.length > 0 && (
                                                    <UiMenuItem onClick={() => selectedRowForMenu && handleOpenPersonnelFilesModal(selectedRowForMenu)}>
                                                        <ListItemIcon><IconFile width={18} /></ListItemIcon> Belge/Dosyaları İndir ({selectedRowForMenu.attachments.length})
                                                    </UiMenuItem>
                                                )}
                                                {hasDownloadPermission && (
                                                    <UiMenuItem onClick={openDownloadChooserForRow}>
