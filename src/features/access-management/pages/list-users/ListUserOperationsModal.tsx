...
import { Button, Dialog, AppBar, Toolbar, IconButton, Typography, List, ListItem, ListItemText, ListItemIcon, Checkbox, CircularProgress, Box, DialogContent, Divider, Grid, Paper, Collapse } from 'src/shared/components/compat';
import { Slide } from 'src/shared/components/compat';
...
const MenuItemRenderer: React.FC<MenuItemRendererProps> = ({
    ...
    const handleMenuSelectionClick = useCallback((event: React.MouseEvent) => {
        if (hasChildren && event.target instanceof Element && (event.target.closest('.UiIconButton-root') || event.target.closest('svg'))) {
                                                    ...
                                                    return (
                                                        ...
                                                        >
                                                            <ListItem
                                                                ...
                                                                sx={{ py: 0.5, '&.Ui-disabled': { opacity: 0.5, pointerEvents: 'none' } }}
                                                            >
                                                                <ListItemIcon>
                                                                    <Checkbox
                                                                        edge="start"
